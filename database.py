...
def _ensure_projection_series_type_check(conn):
    """Widen the CHECK constraint on projection_series.type to include 'investment' and 'rescue'."""
    if backend_name(conn) == "postgresql":
        actual_name = _prefixed_table_name("projection_series")
        # PostgreSQL: drop old check and add new one
        conn.execute(f'ALTER TABLE "{actual_name}" DROP CONSTRAINT IF EXISTS projection_series_type_check')
        conn.execute(f'ALTER TABLE "{actual_name}" DROP CONSTRAINT IF EXISTS oacc_projection_series_type_check')
        conn.execute(
            f"""ALTER TABLE "{actual_name}" ADD CONSTRAINT {actual_name}_type_check
               CHECK (type IN ('income', 'expense', 'investment', 'rescue'))"""
        )
    else:
        # SQLite: recreate table to update CHECK constraint
        try:
            conn.execute("INSERT INTO projection_series (name, type, start_date, months, monthly_amount) "
                         "VALUES ('__type_check_test', 'investment', '2000-01-01', 1, 1)")
            conn.execute("DELETE FROM projection_series WHERE name = '__type_check_test'")
        except Exception:
            # CHECK constraint blocks 'investment' → recreate table
            conn.executescript("""
                CREATE TABLE IF NOT EXISTS projection_series_new (
                    id             INTEGER PRIMARY KEY AUTOINCREMENT,
                    name           TEXT NOT NULL,
                    type           TEXT NOT NULL CHECK(type IN ('income','expense','investment','rescue')),
                    start_date     TEXT NOT NULL,
                ...
                INSERT INTO projection_series_new SELECT * FROM projection_series;
                DROP TABLE projection_series;
                ALTER TABLE projection_series_new RENAME TO projection_series;
            """)
