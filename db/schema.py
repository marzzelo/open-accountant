...
CREATE TABLE IF NOT EXISTS oacc_recurring_transactions (
    id                 INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    debit_account      INTEGER NOT NULL REFERENCES oacc_accounts(id) ON DELETE RESTRICT,
    credit_account     INTEGER NOT NULL REFERENCES oacc_accounts(id) ON DELETE RESTRICT,
    amount             DOUBLE PRECISION NOT NULL CHECK(amount > 0),
    ...
    fx_source          TEXT,
    description        TEXT NOT NULL DEFAULT '',
    alert_day          INTEGER NOT NULL CHECK(alert_day BETWEEN 1 AND 31),
    alert_active       BOOLEAN NOT NULL DEFAULT TRUE,
    enabled            BOOLEAN NOT NULL DEFAULT TRUE,
    last_posted_period TEXT,
    last_posted_at     TIMESTAMP,
    last_transaction_id INTEGER REFERENCES oacc_transactions(id) ON DELETE SET NULL,
    created_at         TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at         TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS oacc_recurring_transaction_tags (
    recurring_transaction_id INTEGER NOT NULL REFERENCES oacc_recurring_transactions(id) ON DELETE CASCADE,
    tag_id                   INTEGER NOT NULL REFERENCES oacc_tags(id) ON DELETE CASCADE,
    created_at               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX IF NOT EXISTS oacc_idx_recurring_enabled ON oacc_recurring_transactions(enabled);
CREATE INDEX IF NOT EXISTS oacc_idx_recurring_alert ON oacc_recurring_transactions(alert_active, alert_day);
...
CREATE INDEX IF NOT EXISTS oacc_idx_recurring_transaction_tags_tag ON oacc_recurring_transaction_tags(tag_id);
