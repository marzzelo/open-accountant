...
Handles:
  ...
  - DATABASE_URL resolution (PostgreSQL required)
  - app settings stored in the main database (``oacc_settings`` table)
"""
...
def load():
    ...
    if not database_url():
        raise RuntimeError(
            f"{DATABASE_URL_ENV} is not set. Open Accountant requires a PostgreSQL "
